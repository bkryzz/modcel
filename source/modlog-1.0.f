...
!-----------------------------------------------------------------------------------
      ...
      integer*4, parameter ::  ntime=10000,nsize=249,ncmax=249*249
!- careful to set always ncmax=(nsize)^2 with nsize=integer+1
      ...
      integer, allocatable :: iphse(:),innb(:,:)
      ...
      dens = 0.d0
      do kc=1,60;ityp(kc)=2;end do    !  make core of tumor cells
...
!     call map(itime,ncmax,ncel,issb,idsb,iloc,ityp,ipsx,ipsy,ipsz)
!- plot map of tracer diffusion
      call map(itime,ncmax,ncel,issb,idens,iloc,ityp,ipsx,ipsy,ipsz)
...
!- cell cycle phase
     ...
     &    idupl(kc)=idupl(kc)-(rand()/10.d0)*11  !  this anticipates slightly the G1 phase
      ...
      if (lc.gt.600)  iphse(kc)=2
      if (lc.gt.1000) iphse(kc)=3
      if (lc.gt.1300) iphse(kc)=4
  66  continue
      write (16,*) itime,kc,lc,iphse(kc),idupl(kc)
      end do
...
!- test for tracer diffusion
      call fick(dens,dffc,itime,ncmax,ncel,iloc,iocc,idens,innb)
...
!-----------------------------------------------------------------
      subroutine fick(dens,dffc,itime,ncmax,numc,iloc,iocc,idens,innb)
!- simulate diffusion of a tracer among cells
      ...
      integer*4 :: idens(ncmax)
      real*8 :: dc, dtot, dffc, dens(ncmax)
