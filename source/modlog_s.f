...
!  This version includes choice of cubic lattice geometry
!  This version also includes OpenMP parallelization
!                                                         .FC 05/2024
!-----------------------------------------------------------------------------------
      ...
      implicit none
      character*40 word(0:10,0:10)
      integer*4, parameter ::  nst=100, ndt=5
      integer*4, parameter ::  ntime=10000,maxn=50,ncmax=110000
      integer*4 :: ncel, ntime_ir, ntime_cy, itconv, kc, kt, kj, kk, lc
      integer*4 :: ikt,jkt,izt,jzt,index,ndupl,iss,itime,jtime,iO2cod
      integer*4 :: isphcub,iphcod,ifood,numf
      integer*4 :: idtype,ictype,iltype,naxn,mcmx,mcel
      ...
      word(0,0) = ' --> COMPLETE RUN                       '
      word(1,0) = ' --> READ BACKUP CONF FROM PREVIOUS CALC'
      word(2,0) = ' --> GENERATE 3-D CONF AND STOP         '
      word(0,1) = ' --> USE SIMPLE SQUARE/CUBIC LATTICE    '
      word(0,2) = ' --> 2-D LATTICE SIMULATION             '
      word(1,2) = ' --> 3-D LATTICE SIMULATION             '
      open(unit=10,file='BACKUP',status='old',form='unformatted')
!--------------------------- end declarations ------------------------------

      if (ndt.gt.nst) stop 999
!- input data (either from line or file)
      idtype=2                      ! dimension of space (2/3 = 2D or 3D)
      ictype=0                      ! type of simulation (0/1/2 = full/restart/makeconf)
      iltype=1                      ! type of lattice (0/1 = Voronoi/cubic)
      ...
      read(5,*) idtype,ictype,iltype,wid,hei,siz,rad,isphcub,rcut,ncel,   &
                dose,ntime_ir,ntime_cy,itconv,ssb,dsb,rssb,rdsb,dupr0,    &
                dffc(1),dffc(2),dffc(3),dffc(7),baseG0,baseO0,g0l,        &
                iphcod,ifood,iO2cod,prtim,prdsb,prdif,prcel,prpha 
      write(*,'(1x,a)')   ' >------ PROGRAM MODLOG ------< '
      write(*,'(1x,a)')   ' >------ V2.3 - 05.2024 ------< '
      write(*,'(1x,a,/)') ' >-- 2.D/3.D CELL SIMULATOR --< '
      write(*,'(1x,a,i2,a)') ' SPACE DIMENSION ',idtype,word(idtype-2,2)
      write(*,'(1x,a,i2,a)') ' TYPE OF RUN ',ictype,word(ictype,iltype)
      write(*,'(1x,a,i2,a)') ' LENGTH UNIT = micrometers (10^-6 m) '
      write(*,'(1x,a,f8.3)') ' FICTIVE CELL SIZE (r) ',rad ! rad=1.5d0*rad-0.4d0
      ...
      if (idtype.gt.2) then 
         ...
         write(*,'(1x,a,i2,a)')   ' BOX SHAPE ',isphcub,' (0=inf, 1=sphere, 2=cube)'
      end if
      if (iltype.eq.0) then
        write(*,'(1x,a,i6)')    ' EXTENDED LATTICE SIZE ',ncmax
        write(*,'(1x,a,i6)')    ' VORONOI LATTICE SIZE ',6*ncmax/10
        write(*,'(1x,a,i6)')    ' MAX NEIGHBOR-LIST SIZE ',maxn
      else
        write(*,'(1x,a,i6)')    ' NEIGHBOR-LIST SQUARE/CUBIC, max ',maxn
      end if
      ...
      write(*,'(1x,a,f12.3)')   ' GLUCOSE EFFECTIVE DIFFUSION CONST ',dffc(2)
      write(*,'(1x,a,i6)')      ' OXYGEN SUPPLY CONDITION ',iO2cod
      write(*,'(1x,a,f12.3,a)') ' OXYGEN BASELINE CONC ',baseO0, ' [M]'
      write(*,'(1x,a,f12.3)')   ' OXYGEN EFFECTIVE DIFFUSION CONST ',dffc(3)
      write(*,'(1x,a,f12.3)')   ' NECROTIC FACTOR DIFFUSION CONST ',dffc(7)
      write(*,'(1x,a,5i4)')     ' PRINT OPTIONS',prtim,prdsb,prdif,prcel,prpha

!-  THIS BLOCK IS REQUIRED ONLY FOR DNA DAMAGE SIMULATIONS  ------------
...
!-  BUILD VORONOI LATTICE (OR READ FROM PREVIOUS RUN) ------------------
      if (iltype.eq.0.and.ictype.eq.1) then       !  READ OLD CONFIGURATION
          read (10) mcmx,numf,naxn,mcel,isphcub,rad,wid,hei,siz,rcut
          if ( mcmx.ne.ncmax .or. naxn.ne.maxn .or. mcel.lt.ncel ) stop 666
          ...
          WRITE(*,'(1x,a,f10.3)') ' RADIUS OF VORONOI SPHERE ',dsqrt(rdi(numf))
!- OR MAKE THE NEW ONE (careful, it can take a lot of cpu time)
      else if (iltype.eq.0.and.ictype.ne.1) then
!- SPACE attributes spatial location to cells (here use Voronoi)
          call space (ncmax,numf,maxn,ncel,ipsx,ipsy,ipsz,iloc,     &
                      iocc,nnab,innb,isphcub,rad,wid,hei,siz,rcut,x,y,z,rdi)
          WRITE(*,'(1x,a,f10.3)') ' RADIUS OF VORONOI SPHERE ',dsqrt(rdi(numf))

          if(ictype.eq.2) then     !  ONLY WRITE CONFIGURATION AND STOP
...
!-  BUILD SIMPLE SQUARE/CUBIC LATTICE (NO READ FROM PREVIOUS RUN) ------
      else if (iltype.gt.0) then
          call sqube (idtype,ncmax,numf,maxn,ncel,ipsx,ipsy,ipsz,iloc,     &
                      iocc,nnab,innb,isphcub,rad,wid,hei,siz,rcut,x,y,z,rdi)
          print *, 'ok';stop
      end if 
...
!     print *, kk,dsqrt(rdi(kk)),suma/dfloat(nnab(kk)),fill
      end do
!------------------------------------------------------------------------

!-  assign initial phase to cells iphc=1 random, =2 sync
...
!--------------------------IRRADIATION PHASE-----------------------------
!- IF NTIME_IR > 0 start irradiation time iteration
!
...
!       if (iphse(kc).eq.1)     &
!           idupl(kc)=idupl(kc)-(rand()/10.d0)*11  !  this randomizes slightly the G1 phase
...
!       jzt=jzt+idsb(kt)
!     end do
!     write(15,*) ntime_ir+itconv*itime,ikt,izt,dfloat(jkt)/dfloat(ncel),dfloat(jzt)/dfloat(ncel) 
...
!- print time summary every PRTIM time steps
      if (mod(itime,prtim).eq.0) write (6,101) itime,ncel,dsqrt(rdi(ncel))
  101 format(1x,70('-'),/,' time=',i6,' population=',i6,' spheroid R=',f8.3,/)
!- print pdb map of ssb and dsb every PRDSB time steps
      if (prdsb.gt.0.and.mod(itime,PRDSB).eq.0)         &
          call map(itime,ncmax,numf,ncel,issb,idsb,iloc,ityp,ipsx,ipsy,ipsz,x,y,z,dens)
!- print map of tracer diffusion every PRDIF time steps
      if (prdif.gt.0.and.mod(itime,PRDIF).eq.1) then
          ...
          call map(itime,ncmax,numf,ncel,issb,idens,iloc,ityp,ipsx,ipsy,ipsz,x,y,z,dens)
          nunn=nunn+1
          write(6,*) ' output radial data on unit ',nunn
          do kk=1,ncel,10
!         write(nunn,*)kk,dsqrt(rdi(kk)),dens(kk,2),dens(kk,3),
          write(nunn,*)   dsqrt(rdi(kk)),dens(kk,2),dens(kk,3),dens(kk,4)/dens(kk,5)
          enddo
          close(nunn)
          do kk=1,ncel
          ...
          elseif (idupl(kk).le.itime.and.idupl(kk).ge.itime-icycle(kk)) then
...
!- print map of cell landscape every PRCEL time steps
      if (prcel.gt.0.and.mod(itime,PRCEL).eq.0)          &
          call map(itime,ncmax,numf,ncel,issb,iocc,iloc,ityp,ipsx,ipsy,ipsz,x,y,z,dens)
!- print map of cell phase every PRPHA time steps
      if (prpha.gt.0.and.mod(itime,PRPHA).eq.0)          &
          call map(itime,ncmax,numf,ncel,issb,iphse,iloc,ityp,ipsx,ipsy,ipsz,x,y,z,dens)
